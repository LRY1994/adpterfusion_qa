    ...
    def load_data(self, sub_set):
        ...
        with open(self.ent_file, "r") as f:
            ...
            for ent in f.readlines():
                self.ent_total += 1
                arr = ent.split("\t")
                randomNumber = random.randint(1,len(arr)-1)
                self.id2ent[arr[0].strip()] = arr[randomNumber].strip()#'</s>'.join(ent.split("\t")[1:]).strip()#ent.split("\t")[1]
            print(
        ...
        with open(self.rel_file, "r") as f:
            ...
            for rel in f.readlines():
                ...
                arr = rel.split("\t")
                randomNumber = random.randint(1,len(arr)-1)
                self.id2rel[arr[0].strip()] = arr[randomNumber].strip()
